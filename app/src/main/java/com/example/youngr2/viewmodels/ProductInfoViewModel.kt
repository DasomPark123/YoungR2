...
class ProductInfoViewModel(private val productInfoRepository: ProductInfoRepository) : ViewModel() {
    ...
    private var currentSearchResult : Flow<PagingData<ProductListItemModel>>? = null

    fun requestProductInfo(product : String) : Flow<PagingData<ProductListItemModel>> {
        ...
        val newResult : Flow<PagingData<ProductListItemModel>> = productInfoRepository.getProductInfo(product).cachedIn(viewModelScope)
